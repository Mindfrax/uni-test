//fail currently at line 110 
//not usable at the moment Date 7.02.2021
//need to view private library
...
function write_state($state){
    file_put_contents('tmp/oauthtestentry.php', serialize($state));
}
...
if(empty($_GET['reset']) && file_exists('tmp/oauthtestentry.php')){
    $state = read_state();
}
else{
    $state = array();
